...
/**
 * The one {@link ContainersClient} this process has, and where its bearer comes from. A copy of
 * qits-ci's producer of the same name, which stays the reference.
 *
 ...
 * which is what leaves this decision here. The defaults ship at ordinal 100 in the client jar, so
 * the lines below name keys rather than values.
 *
 ...
 * {@code …AndAwait} spelling hides — would park it on one slow token endpoint. {@link TokensHelper}
...
public class ContainersClientProducer {
   ...
   * 401 is a better answer for a panel than an unbounded wait.
   */
