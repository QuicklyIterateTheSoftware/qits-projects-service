 ...
 * the next start, so no deployment names it, no manifest pins it and no access record keeps it warm.
 * qits-configuration answers the <em>configured</em> versions; this route answers the
 * <em>effective</em> ones — the values this running process resolved at boot, which lag the
 * configured entry until this service is deployed again. That lag is why the route exists and why it
 * reads its own config rather than asking qits-configuration: a peer's answer would be the version
 ...
 * bare name would resolve against whatever lies in the host daemon's local store, while the GC names
 * images as the registry holds them ({@code qits/project-agent}), so a leading host segment is
 * stripped here. A value already registry-relative passes through unchanged.
 *
 * <p><b>A blank version omits the row and an empty answer is a valid 200.</b> The reply says what
 * would be pulled; a half-composed reference names nothing and must not reach a keep-rule as if it
 * did. Refusing instead would fail the GC closed over a service that simply launches nothing.
 *
 * <p><b>A machine route that an operator may also read</b>: qits-platform-orchestrator asks it with a
 * bearer, and someone asking what the GC will be told is asking the same question — hence {@code
 * qits:admin} beside {@code qits:system}. Nothing needs registering for reflection: the method
 * returns its record type rather than a bare {@link jakarta.ws.rs.core.Response}, so the native build
 * indexes it.
 */
@Path("/pins")
...
public class PinsController {
   ...
   * @param launches what starting it would be — {@code agent} or {@code refinement}
   */
  ...
  public LaunchPins pins() {
    return new LaunchPins(
        Instant.now(),
        pins(
            agentContainers.imageRepo(),
            agentContainers.imageVersion(),
            refinementContainers.imageRepo(),
            refinementContainers.imageVersion()));
  }

  /**
   * The rows, ordered by image and then by what launches it. Deterministic because the consumer diffs
   * one run's answer against the next, and package-private because the omission rules are worth
   * proving without a config profile per case.
   */
  static List<LaunchPin> pins(
      String agentRepo, String agentVersion, String refinementRepo, String refinementVersion) {
    return Stream.of(
            pin(agentRepo, agentVersion, "agent"),
            pin(refinementRepo, refinementVersion, "refinement"))
   ...
   * registry.dev.localhost:8080/qits/project-agent} is {@code qits/project-agent}, and {@code
   * qits/workspace} is already what the registry calls it.
   */
